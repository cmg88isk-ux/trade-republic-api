  ...
  for (let i = 0; i < 8; i++) {
    ...
    signals.push({
      ...
      rationale: signalType === 'BUY' 
        ? `TR price ${Math.abs(spreadPct).toFixed(1)}% below Binance reference. Strong buy opportunity.`
        : signalType === 'SELL'
        ? `TR price ${Math.abs(spreadPct).toFixed(1)}% above Binance. Consider selling.`
        : `Spread near threshold. Monitoring for entry point.`,
      suggestedAction: signalType === 'BUY' ? 'Buy on Trade Republic' : signalType === 'SELL' ? 'Sell on Trade Republic' : 'Monitor closely',
...
export default function SignalsPage() {
  ...
  const acknowledgeSignal = (signalId: string) => {
    setSignals(prev => 
      prev.map(s => 
        s.id === signalId ? { ...s, status: 'ACKNOWLEDGED' } : s
      )
    );
  };
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="font-medium text-[var(--negative)]">
                  {criticalSignals.length} Critical Signal{criticalSignals.length > 1 ? 's' : ''} Detected
                </div>
