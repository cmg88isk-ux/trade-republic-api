...
// Mock TR prices
const MOCK_TR_PRICES: Record<string, { price: number; bid: number; ask: number; change24h: number }> = {
...
export default function MarketsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setMarketData({
        ...
        btcPrice: snapshot.btcUsdt.price || 95000,
      });
    } catch (error) {
  ...
  const allInstruments: (TRInstrument & { source: 'TR' | 'BINANCE'; priceData?: typeof MOCK_TR_PRICES[string] })[] = [
    ...TR_GOLD_INSTRUMENTS.map(i => ({ ...i, source: 'TR' as const, priceData: MOCK_TR_PRICES[i.isin] })),
    ...TR_SILVER_INSTRUMENTS.map(i => ({ ...i, source: 'TR' as const, priceData: MOCK_TR_PRICES[i.isin] })),
  ];
        ...
        <div className="terminal-panel p-4">
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              >
                Silver ({silverInstruments.length})
              </Button>
            </div>
                ...
                {filteredInstruments.map((instrument) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-3 text-right">
                        <span className="font-mono text-xs text-muted-foreground">
                          {instrument.gramPerUnit?.toFixed(3) || '--'}
                        </span>
                      </td>
                    </tr>
                  );
