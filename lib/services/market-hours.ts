...
export function getTimeUntilTROpen(date: Date = new Date()): {
  ...
  while (!config.tradingDays.includes(nextOpen.getDay()) || isGermanHoliday(nextOpen)) {
    nextOpen.setDate(nextOpen.getDate() + 1);
  }
...
export function getMarketStatusText(date: Date = new Date()): string {
  ...
  switch (trState) {
    case 'OPEN':
      ...
      return `Market Open - Closes in ${closeInfo.formattedDuration}`;
    case 'CLOSED':
      const openInfo = getTimeUntilTROpen(date);
      return `Market Closed - Opens in ${openInfo.formattedDuration}`;
    case 'PRE_MARKET':
      const preOpenInfo = getTimeUntilTROpen(date);
      return `Pre-Market - Opens in ${preOpenInfo.formattedDuration}`;
    case 'AFTER_HOURS':
      const afterOpenInfo = getTimeUntilTROpen(date);
      return `After Hours - Opens in ${afterOpenInfo.formattedDuration}`;
    default:
