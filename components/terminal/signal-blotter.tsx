...
import type { TradingSignal, SignalType, AssetType } from '@/lib/types/arbitrage';
...
function getPriorityBadge(priority: TradingSignal['priority']) {
  const variants: Record<typeof priority, { class: string; label: string }> = {
    CRITICAL: { class: 'bg-[var(--negative)] text-white', label: 'CRITICAL' },
    HIGH: { class: 'bg-[var(--warning)] text-black', label: 'HIGH' },
    NORMAL: { class: 'bg-muted text-foreground', label: 'NORMAL' },
    LOW: { class: 'bg-surface-2 text-muted-foreground', label: 'LOW' },
  };
...
function SignalItem({
  ...
  const timeAgo = formatDistanceToNow(new Date(signal.timestamp), { addSuffix: true });
    ...
    >
      ...
      <div className="flex items-center justify-between mt-3">
        <div className="flex items-center gap-3 text-xs font-mono">
          <span>
            Spread: <span className="text-foreground">{signal.spreadPct > 0 ? '+' : ''}{signal.spreadPct.toFixed(2)}%</span>
          </span>
          <span>
      ...
      {showHeader && (
        <div className="terminal-panel-header">
          <div className="flex items-center gap-2">
            ...
            <span className="terminal-panel-title">Signals</span>
          </div>
          {newSignals.length > 0 && (
