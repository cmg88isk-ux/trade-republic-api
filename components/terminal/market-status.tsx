    ...
    <div className={cn('terminal-panel', className)}>
      <div className="terminal-panel-header">
        <div className="flex items-center gap-2">
          ...
          <span className="terminal-panel-title">Market Status</span>
        </div>
      ...
      <div className="terminal-panel-content space-y-3">
        ...
        <div className="pt-2 border-t border-border-subtle">
          <div className="flex items-center justify-between">
            <span className="text-sm">Prices Comparable</span>
            {isComparable ? (
              <span className="text-sm font-medium text-[var(--positive)]">YES</span>
            ) : (
              <span className="text-sm font-medium text-[var(--negative)]">NO</span>
            )}
          </div>
        </div>
          ...
          <>
            ...
            {isWeekendGap && (
              <div className="flex items-center gap-2 p-2 rounded bg-[var(--warning)]/10 text-[var(--warning)]">
                ...
                <span className="text-xs">Weekend gap period - monitor for gap trades</span>
              </div>
            )}
          </>
        )}
...
export function MarketStatusBadge({ className }: MarketStatusBadgeProps) {
  ...
  return (
    ...
    >
      ...
      />
      {trState === 'OPEN' && 'Market Open'}
      {trState === 'CLOSED' && 'Market Closed'}
      {trState === 'PRE_MARKET' && 'Pre-Market'}
      {trState === 'AFTER_HOURS' && 'After Hours'}
    </div>
  );
}
